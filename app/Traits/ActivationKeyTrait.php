...
trait ActivationKeyTrait
{
    ...
    public function createActivationKeyAndNotify(User $user)
    {
        ...
        $user->notify(new ActivationKeyCreatedNotification($activationKey,  $user));
    ...
    public function processActivationKey(ActivationKey $activationKey){
        ...
        if ($userToActivate->activated){
            return false;
        }

        // set the user to activated
        $userToActivate->activated = true;
        $userToActivate->activation_time = Carbon::now();
        $userToActivate->save();

        $activations = ActivationKey::where('user_id',$activationKey->user_id)->first();
        $activations->activation_time = Carbon::now();
        $activations->save();

        \Auth::loginUsingId($activationKey->user_id);
